...
第五章的重点是理解“语法背后仍然是函数调用”。运算符重载让自定义类型更像内置类型，但前提是语义要自然。友元可以解决类外函数访问私有成员的问题，但要控制使用范围。函数指针、成员函数指针、函数对象和 Lambda 都属于可调用实体，它们让函数也可以像数据一样被传递和保存。RAII 和单例自动释放则强调：资源应该交给对象生命周期去管理，而不是依赖手动记忆释放。
