...
# 实时追踪并查看日志文件的最新追加内容
tail -f app.log  
# 在 app.log 中搜索包含 "error" 的行，并显示对应的行号
grep -n "error" app.log 
# 逐行读取 access.log，并打印每行的第一个字段
awk '{print $1}' access.log 
# 将 file 文件中所有匹配到的 "foo" 替换为 "bar" 并输出
sed 's/foo/bar/g' file
# 对 file 文件的内容进行排序，然后统计每行连续出现的次数
sort file | uniq -c
```
...
# 更改 app 文件的权限
chmod 755 app
# 修改 file 文件的所有者为 "user"，所属组为 "group"
chown user:group file
# 显示当前用户的 UID（用户ID）、GID（组ID）以及所属的用户组信息
id
# 显示当前登录系统的有效用户名
whoami
# 以系统超级用户（root）的权限来执行后面的 command 命令
sudo command
```
...
```bash
# 列出系统当前所有正在运行的进程的详细快照信息
ps aux
# 动态、实时地显示系统整体资源使用情况（CPU、内存）以及各个进程的运行状态
top
# 实时查看指定进程（PID）内部所有线程的资源占用和运行状态（-H 表示显示线程）
top -H -p <pid>
# 每隔 1 秒读取一次指定进程（PID）的 CPU、内存等监控数据
pidstat -p <pid> 1
# 向指定进程发送 SIGTERM 信号，通知其安全、优雅地终止
kill -TERM <pid>
# 向指定进程发送 SIGKILL 信号，强制、立即终止该进程
kill -9 <pid>
```
...
```bash
# 查看系统当前正在监听（l）的 TCP（t）和 UDP（u）端口
# 以数字形式显示地址和端口（n），并显示对应的进程名及 PID（p）
ss -lntup
# 访问指定网址并输出详细的请求过程（-v 表示 verbose）
curl -v http://example.com
# 向目标主机（IP 或域名）发送 ICMP 回显请求
# 用于测试网络连通性以及计算往返时延（RTT）
ping host
# 追踪数据包从本地到目标主机所经过的所有路由节点（路由器 IP）
# 用于定位网络在哪里出现了延迟或断开
traceroute host
# 捕获并打印指定网卡(eth0)上通过 80 端口(通常是 HTTP 流量)的所有网络数据包
tcpdump -i eth0 port 80
# 列出当前正在占用 8080 端口的所有进程信息
lsof -i :8080
```
...
# 以人类易读的格式（-h，如 GB、MB）显示容量、已用和剩余
df -h
# 统计当前目录下各个文件和子目录的总大小
# -s 表示汇总，-h 表示易读格式
du -sh *
# 以树状列出系统中的所有块设备（硬盘、分区、光驱等）及其挂载点、大小和架构关系
lsblk
# 每隔 1 秒输出一次磁盘 I/O 的详细扩展统计信息（-x）
# 用于监控磁盘读写吞吐量、利用率（%util）和等待时间，判断是否存在 I/O 瓶颈
iostat -x 1
# 查看系统内存（RAM）和交换分区（Swap）的整体使用情况
free -h
# 每隔 1 秒输出一次虚拟内存、进程、CPU 活动和系统整体状态的统计信息
# 是判断系统整体性能瓶颈（CPU 还是 I/O）的轻量级利器
vmstat 1
# 显示指定进程（PID）的详细内存映射布局（-x 表示扩展格式）
# 展示该进程各个内存段（如堆、栈、共享库）的占用情况，常用于排查内存泄漏
pmap -x <pid>
```
...
内存泄漏指的是程序在运行过程中**申请了动态内存，但在不再使用时没有正确释放**，导致这些内存无法被再次利用，最终引起进程内存持续增长，甚至耗尽系统资源。
在 C++ 中，内存泄漏通常来自 `new/malloc` 后没有 `delete/free`，也可能来自对象生命周期设计不合理，比如 `shared_ptr` 循环引用、全局缓存只增不删、回调长期持有对象等。严格来说，文件描述符、socket、线程、数据库连接未关闭属于资源泄漏，但在工程实践中也会和内存泄漏一起排查。
常用检查工具包括 ASan、LSan、Valgrind 等。线上环境还可以通过 `top`、`pmap`、`/proc/<pid>/smaps` 等方式观察进程内存增长趋势，但这些方法只能辅助判断，不能精确定位泄漏代码。
...
需要注意：
- `top`、`pmap`、`smaps` 主要用于观察进程内存是否持续增长。
- Valgrind 能比较准确地定位未释放内存，但运行速度较慢。
- ASan/LSan 运行速度比 Valgrind 快，适合开发和测试阶段使用。
- ASan 和 Valgrind 一般不要同时使用。

#### 常见泄漏场景

- `new/malloc` 后异常路径忘记释放。
- 函数中提前 `return`，导致释放逻辑没有执行。
- 容器或全局缓存只增不删。
- `shared_ptr` 循环引用，导致引用计数永远不为 0。
- 线程、文件描述符、socket、数据库连接未关闭。
- 回调、lambda、异步任务持有对象，导致对象生命周期被意外延长。
- 手写资源管理类时，拷贝构造、赋值运算符、析构函数处理不当。
...
- 用对象生命周期管理资源，即 RAII。
- 尽量避免手动 `new/delete`，优先使用智能指针和标准容器。
- 独占资源用 `unique_ptr`。
- 共享资源谨慎用 `shared_ptr`，避免循环引用。
- 需要反向引用或观察关系时，用 `weak_ptr`。
- C 资源使用自定义 deleter 包装成 RAII 对象。
- 代码评审时重点关注资源所有权、异常路径、提前返回路径。
- 对长期运行的服务，要关注缓存淘汰策略和对象生命周期，避免“逻辑泄漏”。
